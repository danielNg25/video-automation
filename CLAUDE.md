...
Four-stage pipeline: **Download → Transcribe → Process → Upload**

- **Download**: Douyin API (self-hosted Evil0ctal container on :8080) with yt-dlp fallback
- **Transcribe**: mlx-whisper on macOS, faster-whisper on Linux — auto-selected via `sys.platform`
...
Data flows through: `data/raw/` → `data/srt/` → `data/output/` → `data/logs/`

Each video's pipeline state is persisted to `data/logs/{video_id}_state.json` for crash recovery.
...
- **Subtitle language per platform**: Chinese for TikTok, English for YouTube/Facebook/X. Configured in `config/platforms.yaml`.
- **Stage-level state persistence**: Pipeline saves progress after each stage (download/transcribe/process/upload) so interrupted runs resume from the last completed stage, not from scratch.
