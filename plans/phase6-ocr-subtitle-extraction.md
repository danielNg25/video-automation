...
#### 6.1 Add PaddleOCR dependencies — `pyproject.toml`

Add `paddlepaddle>=2.6.0` and `paddleocr>=2.7.0` to base dependencies.

**Dependencies**: none.

#### 6.2 Add `extract_frames()` — `src/processor/ffmpeg.py`
...
#### 6.3 Create `OCRTranscriber` — `src/transcriber/ocr.py`
...
#### 6.4 Extend transcriber factory — `src/transcriber/__init__.py`
...
#### 6.6 Extend API models — `src/api/models.py`
...
#### 6.7 Update router + task manager
...
#### 6.10 Update DownloadTranscribe page
...
### V6.1: PaddleOCR installed

```bash
python3 -c "from paddleocr import PaddleOCR; print('OK')"
```

### V6.2: Frame extraction works
...
### V6.3: OCR transcription produces SRT
...
### V6.4: Whisper backward compatibility
