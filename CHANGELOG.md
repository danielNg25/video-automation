...
### Changed
- OCR engine swapped from PaddleOCR to RapidOCR (ONNX Runtime, same PP-OCRv4 weights bundled in the python package). Removes the Paddle 3.x PIR/OneDNN crash workaround (`paddle.set_flags(...)`) and the ~600 MB PaddlePaddle install. Added frame-diff skipping: each frame's bottom strip is compared (mean abs pixel diff) against the previous one before being sent to OCR — identical-looking frames reuse the cached result. The two-pass (sample-every-5th + full) scan is replaced by a single streaming pass with a rolling 50-frame watermark buffer refreshed every 10 OCR calls. Combined effect: typical ~5–15× wall-time reduction on CPU; larger when an NVIDIA GPU is wired in via the new `[gpu]` extra (`pip install -e ".[gpu]"` + `docker run --gpus all`). New ocr config knobs: `enable_frame_diff` (default `true`), `frame_diff_threshold` (default `3.0` on 0–255 scale), `execution_provider` (`auto | cpu | cuda | directml | coreml`). `_parse_ocr_result` gained a RapidOCR-shape branch; the PaddleOCR v2/v3 branches stay as fallbacks. The `paddleocr_cache` Docker volume becomes orphaned — see DOCKER.md for cleanup. SRT format, `_ocr_meta.json` shape, manual-region path: unchanged.
