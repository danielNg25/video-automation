...
### Changed
- **Subtitle versioning + dub picks a version (replaces the implicit Sync Dub flow).** The editor saves to a mutable working draft; a new "Save as version" button snapshots it as the next auto-numbered immutable version (v1, v2, …) with an optional friendly name. DubTab gains a "Subtitle version" picker; each (version + provider + voice) gets its own dub WAV in the audio library. Per-(video, language) `versions.json` indexes snapshots. Migration is silent on first read: legacy `.dubsync.srt` becomes `v1`, existing dub WAVs gain a `_v1_` infix, `dub_meta_*.json` and the per-segment WAV cache are deleted. **Deleted:** `src/tts/sync_runner.py`, `src/tts/dub_meta.py`, `src/tts/dubsync_srt.py`, `src/tts/segment_cache.py`, `assembler.run_partial`, `POST /api/videos/{id}/dub/sync`, the Sync Dub banner in EditorTab, `_check_dub_sync_against_meta`, `postDubSync` FE client. **Added:** `src/api/versions.py` + `src/api/routers/versions.py` (CRUD), `ui-app/src/api/versions.ts`, `useVersions` hook, `VersionPicker`, `VersionPanel`. 25 new BE tests (`tests/test_versions.py`, `tests/test_migration.py`, `tests/test_srt_versioned.py`, `tests/test_tts_versioned.py`) + 13 new FE vitest tests.

### Fixed
- **DubTab voice list stuck on "Loading voices…" on first open.** The "remove voice profiles" refactor (PR #12) deleted the implicit voice-load that used to happen as a side effect of profile loading and never replaced it. On a fresh `VideoDetail` mount, `loadVoicesForProvider` was only triggered by user-initiated picker changes, so a first-time visitor to the Dub tab saw an empty voice list forever. Added a dedicated `useEffect` in `ui-app/src/pages/VideoDetail.tsx` that calls `loadVoicesForProvider` whenever `selectedTtsProvider` or `ttsApiKey` changes — fires once on mount with the default `google` provider, again when the per-provider API key finishes loading from localStorage, and on every subsequent provider switch.

- **Subtitle editor: time inputs are now typable and the add button is discoverable.** The startTime input and the text area both had `onClick={() => onSeek(...)}` — every click jumped the video, which stole focus from the input and made typing impossible. Both handlers removed; seeking is now only triggered by clicking the row's `#` index label (its title was updated to surface the affordance). Time inputs are now controlled by a new `TimestampInput` subcomponent in `ui-app/src/components/editor/SegmentList.tsx`: typing updates state live, invalid format paints a red border (`border-red-400/60`) and surfaces the expected format in the tooltip, blur or Enter commits when valid, Esc reverts. The parent enforces `start < end` via a commit callback that returns a boolean; rejected values revert without a stale display. A `committedValue` derived-state pattern (the React docs' "adjust state during render") replaces the prop-sync `useEffect`, satisfying `react-hooks/set-state-in-effect`. The per-row action toolbar (split / merge / add / delete) is now always visible on the active row (kept hover-only on inactive rows so long lists stay calm). A new sticky **+ Add subtitle** button below the list always shows; it works on an empty list via an `afterIndex < 0` guard added to `EditorTab.handleAddSegment` (start=0, end=min(2, `playerState.duration`), inserted at index 0). 17 new vitest tests at `ui-app/src/components/editor/__tests__/SegmentList.test.tsx`.
...
### Added
- Settings → Douyin Cookie now syncs to **both** cookie files in one save. The existing `/api/settings/cookie` PUT endpoint was extended to splice the same cookie value into the `Cookie:` line of `config/douyin_web_config.yaml` (the evil0ctal helper API's bind-mounted config), in addition to the existing write to `config/douyin_cookie.txt` (yt-dlp fallback). The splice is a regex line-rewrite — comments and other YAML fields are preserved (no `yaml.dump` round-trip stripping). On fresh machines (especially WSL2 where editing the file with nano is painful), the workflow becomes: `make docker-up` → open Settings → paste cookie → save → `docker compose restart douyin-api`. No more hand-editing the YAML. `CookieStatus` model + TS type gain a `helper_config_synced` boolean; the Settings UI surfaces a `HELPER-API SYNCED` / `HELPER-API NOT SYNCED` badge and updates the save-confirmation message to remind users to restart `douyin-api`. Unit tests at `tests/test_settings_cookie.py` cover the splice helper and the placeholder/empty-value detection.
...
### Removed
- Voice profiles system. The UI fetches voices live from each provider's API (`GET /api/tts/voices`) and persists a per-provider `voice_id` in localStorage; the YAML-backed profile bundles `{provider, voice, speed, pitch, language}` were vestigial — the FE always overrode the profile's voice, and the profile's `speed`/`pitch` defaults weren't exposed in the FE at all. Deletions: `config/tts_voices.yaml`, `load_voice_profiles()`/`save_voice_profiles()` in `src/tts/__init__.py`, `GET/PUT/DELETE /api/tts/profiles`, `GET /api/tts/platforms`, `VoiceProfileConfig` and `TTSPlatformConfig` Pydantic + TS types, `voice_profile` field on `TTSRequest` (now `provider` + `voice` are required), `tts_profile` field on `FullPipelineRequest` / `BatchPipelineRequest` (now uses `tts_voice` + `tts_provider` + `tts_language`), DubTab's profile dropdown, and the CLI `--tts <profile>` option (replaced with `--tts-voice / --tts-provider / --tts-language`). `run_tts_track` and `tts_output_path` now take `voice` + `provider` directly. The dub filename pattern changes from `{id}_{lang}_{provider}_{profile_name}.wav` to `{id}_{lang}_{provider}_{voice_id}.wav`. The platform-multi-mix helper `_build_tts_mix_settings` (a leftover from the now-removed social-upload era) is replaced with a single-platform `_default_tts_mix_for_platform(underlay_db)`.

### Changed
- OCR engine swapped from PaddleOCR to RapidOCR (ONNX Runtime, same PP-OCRv4 weights bundled in the python package). Removes the Paddle 3.x PIR/OneDNN crash workaround (`paddle.set_flags(...)`) and the ~600 MB PaddlePaddle install. The two-pass (sample-every-5th + full) scan is replaced by a single streaming pass with a rolling 50-frame watermark buffer refreshed every 10 OCR calls. Typical ~5× wall-time reduction on CPU; larger when an NVIDIA GPU is wired in via the new `[gpu]` extra (`pip install -e ".[gpu]"` + `docker run --gpus all`). New ocr config knob: `execution_provider` (`auto | cpu | cuda | directml | coreml`). `_parse_ocr_result` gained a RapidOCR-shape branch; the PaddleOCR v2/v3 branches stay as fallbacks. The `paddleocr_cache` Docker volume becomes orphaned — see DOCKER.md for cleanup. SRT format, `_ocr_meta.json` shape, manual-region path: unchanged.

### Removed
- Frame-diff skipping in OCR (`_FrameDiffer`, `enable_frame_diff`, `frame_diff_threshold`). Empirical skip rates on real Douyin videos were modest (~8% at fps=1, ~11% at fps=2): the subtitle occupies a small fraction of the frame strip while the video content underneath changes constantly, so the diff was mostly measuring background motion. The engine swap to RapidOCR delivered the bulk of the speedup; the frame-diff added complexity without a meaningful payoff at typical sampling rates. Tests in `tests/test_ocr_frame_diff.py` deleted.
...
- `delete_video` extended to clean up `data/preview/{id}_*.mp4` cache files.
...
- `DubStatusEntry` model + `VideoResponse.dub_status: list[DubStatusEntry]` field. Each entry: `{language, out_of_sync, last_synced_at}`. Populated by enumerating `data/tts/{video_id}/dub_meta_*.json` files and cross-referencing `PipelineState.dub_out_of_sync_languages`. Lets the editor render the Sync-Dub banner without an extra request.
- `_build_dub_status(video_id)` helper in `src/api/task_manager.py`. Reads dub_meta files + state, returns sorted entries per language.
- `src/tts/sync_runner.py`: partial-regen orchestrator with `> 50%` dirty / segment-count-change / provider-voice-speed-underlay-mismatch fallbacks. `identify_dirty_segments` (pure) and `should_fall_back_to_full_regen` (pure) are unit-tested. Falls back to `src.tts.runner.run_tts_track` for full regen, ensuring identical output to the per-video TTS endpoint.
- `Assembler.run_partial(...)`: entry point accepting `clip_overrides: dict[int, Path]` of pre-loaded cached clips for non-dirty segment indices and `dirty_indices: list[int]` for the ones to re-synthesise. Mirrors Stages 1–7 of `generate_full_track` but skips Stage-0 LLM sentence-merging, building one `SentenceGroup` per saved-SRT segment so the per-index alignment with `dub_meta.segment_texts` is preserved. Re-writes the segment cache for newly synthesised clips, re-emits `dubsync.srt`, and updates `dub_meta`.
- `POST /api/videos/{id}/dub/sync` endpoint. Body: `{language, provider, voice_id, playback_speed, underlay_db, api_key}`. Returns `{task_id, status}`. Reads current SRT (preferring `dubsync.srt`) to derive `new_texts`, kicks off a background task that runs `run_dub_sync`, emits SSE progress events, and clears `PipelineState.dub_out_of_sync_languages[language]` on success.
- `SyncDubRequest` Pydantic model.
...
### Fixed
- Editor Sync-Dub banner now appears for legacy dubs (generated before the dub-sync feature shipped — no `dub_meta_{lang}.json` exists). `_build_dub_status` now enumerates languages from BOTH `dub_meta_*.json` files AND `dubsync.srt` files, and flags any language with a dubsync.srt but no dub_meta as `out_of_sync: true` so the user can re-sync (which populates the cache + meta).
- Editor language picker no longer shows a duplicate "vi.dubsync" entry alongside "vi". `task_manager.scan_existing_videos` was parsing `{id}_vi.dubsync.srt` as language `"vi.dubsync"` via `f.stem.split("_")[-1]`; now strips the `.dubsync` suffix and deduplicates via set comprehension before sorting. The same parsing pattern was audited in `transcribe.py` (no occurrences found).
- EditorTab: removed the anachronistic "You're editing the dub-synced subtitle" warning banner. Editing the dubsync file is the canonical workflow; the Sync-Dub button is the explicit mechanism for keeping the dub in sync with edits. The `isDubsync` state and its two setter calls were also removed (the state had no remaining consumers after the banner was deleted).
- Sync-Dub partial path now constructs a translator for Stage 3 LLM shortening (was silently skipping shortening because `TTSAssembler()` was instantiated without a `translator=` kwarg). The endpoint accepts optional `llm_api_key` and `llm_backend` fields on the request body; falls back to `config.translation` if not provided. If no LLM key is available, Stage 3 is skipped with a warning log (graceful degradation).
- TTS segment cache: preserve the source file's extension (typically `.mp3`) when caching, instead of hardcoding `.wav` regardless of actual content. The previous `.wav`-named files contained MP3 bytes — functional (ffmpeg sniffs headers) but confusing. `cache_path_for_segment` is replaced by `segments_dir_for_video` + `cache_basename_for_segment` + extension-agnostic `load_segment_clip` (globs for any ext).
...
- `ui-app/src/pages/videoDetail/ExportTab.tsx`. Standalone Export tab — subtitle-language picker, TTS dub picker, video/dub volume sliders, Preview 5s + Export buttons with progress bar, and the exported-video view modal previously buried inside SubtitleEditorPanel. Drives the existing single-export `/api/videos/{id}/export` endpoint (not platform-based — VideoDetail's old flow was always one full export).
- `ui-app/src/pages/videoDetail/DubTab.tsx`. Lifts the TTS dubbing UI out of VideoDetail's legacy fragment. Adds a "Save current as default" button that writes the current provider / voice / language / playback-speed / underlay-dB values back to the shared localStorage keys (`tts_selected_provider`, `tts_voice_id_<provider>`, `tts_language`, `tts_playback_speed`, `tts_underlay_db`) — establishing the single source of truth that Settings + Pipeline + VideoDetail all read from. Per-job edits remain (the button is opt-in). Existing TTS list with play/delete/preview + provider/voice selectors all behave identically to the legacy panel.
...
### Removed
- `ui-app/src/components/SubtitleEditorPanel.tsx` (592 lines). VideoDetail no longer embeds the subtitle editor; the Overview tab links to the standalone `/editor/:videoId` page via the "Open Editor" button, and the export controls that were buried inside SubtitleEditorPanel now live in the new `ExportTab`. The editor primitives in `ui-app/src/components/editor/` (VideoPlayer, Timeline, SegmentList, StylePanel, etc.) remain — they're consumed by the standalone editor page.
- VideoDetail dead state freed by the migration: `srtSegments` / `setSrtSegments`, `previewLanguage` / `setPreviewLanguage`, the `loadSrt` callback, `handleEditorReload`, the unused `formatDuration` helper, and the `PLATFORM_INFO` constant. Also dropped the SRT-preview side effects from `handleTranscribe` and `handleTranslate` (the editor was the only consumer of those segments). VideoDetail's legacy `activeTab === 'export'` fragment is gone; the tab now renders `<ExportTab />` directly.
