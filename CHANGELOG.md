...
### Changed
- **TTS Stage 3 shortening is now iterative.** The assembler's batched LLM-shortening call (`_apply_shortening`) loops up to `SHORTENING_MAX_PASSES = 3` total passes. Pass 1 hits planner-flagged sentences with the planner's chosen target; passes 2–3 re-derive targets from sentences whose current clip still satisfies `clip_duration / effective_speed > sp.final_duration` and tighten `shorten_pct` one notch down `SHORTEN_TARGETS` (0.85 → 0.75 → 0.65 → floor 0.60) via `planner._tighten` before re-batching. The pass body was extracted into `_run_shortening_pass`. Slots that converge to a fitting clip have their stale `needs_review` cleared; slots still overrunning at the floor are flagged with reason `shorten_floor_reached`. Resolves the case where Vietnamese expansions overran the source span after a single shortening pass even at `playback_speed=1.5×`. Five new unit tests in `tests/test_tts.py::TestIterativeShortening` cover the constant, single-pass convergence, multi-pass iteration with tightened target, the floor cap, and the no-translator early return.
